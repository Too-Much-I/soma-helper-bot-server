...
**① Webex에서 `soma_helper@webex.bot` 검색**
**② 봇에게 DM 보내기 → 끝!**

DM 창에서 아래 명령어를 바로 입력하면 됩니다.

> 💡 스페이스(대화방)에서도 사용 가능 — 봇을 멤버로 초대한 후 `@soma_helper` 멘션과 함께 명령어를 입력하세요.
...
입력한 질문과 기존에 등록된 FAQ 질문들을 비교하여, 유사도가 높은 답변 3개를 추천합니다.
...
**응답 예시:**
입력한 질문과 유사한 FAQ를 찾았습니다.

```
[유사도 0.91]
Q. 부산센터에 수면 시설이 있나요?
A. 잠시 쉴 수 있는 휴게공간은 제공될 예정이나, 숙박 용도로는 사용할 수 없습니다.
[유사도 0.84]
Q. 부산센터에 샤워 시설이 있나요?
A. 센터 내 샤워 시설은 건물 측과 협의 중입니다.
[유사도 0.72]
Q. 부산센터 좌석은 고정석인가요?
A. 개발 공간은 자율적으로 이용 가능합니다. 좌석 수는 48석 이상입니다.
```